    ...
    public function index(Request $request)
{
    ...
    $perPage  = min($request->input('per_page', 25), 500); // max 500 untuk cegah overload

    // Pisahkan sort menjadi field dan arah
    [$sortField, $sortDirection] = explode('_', $sort) + [null, null];
    $sortField     = $sortField ?? 'no_induk';
    $sortDirection = in_array($sortDirection, ['asc', 'desc']) ? $sortDirection : 'asc';
    ...
    $query = \App\Models\Eksemplar::with('inventori')
        ->when($search, function ($q) use ($search) {
            $q->whereHas('inventori', function ($sq) use ($search) {
                $sq->where('judul_buku', 'like', "%{$search}%")
    ...
    switch ($sortField) {
        case 'judul':
            $query->orderBy(
                \App\Models\Inventori::select('judul_buku')
                    ->whereColumn('inventori.id', 'eksemplar.id_inventori'),
                $sortDirection
            );
            break;
    ...
    $eksemplar = $query->simplePaginate($perPage)->appends($request->all());
