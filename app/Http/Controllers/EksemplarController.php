...
class EksemplarController extends Controller
{
    ...
    public function cetakBatch(Request $request)
    {
        ...
        Log::info('CetakBatch Request:', compact('ids', 'startRow', 'endRow', 'search', 'category', 'tanggal', 'sort'));
        ...
        if (!empty($ids)) {
            ...
            try {
                ...
                Log::error('CetakBatch gagal di mode checkbox', ['error' => $e->getMessage()]);
            ...
            try {
                ...
                // Ambil ID dengan aman pakai chunk supaya tidak error di offset besar
                $idsForRange = collect();
                $remaining   = $take;
                $currentRow  = $startRow;
    
                while ($remaining > 0) {
                    $batchSize = min($remaining, 200); // batch aman
                ...
                foreach ($eksemplarList->pluck('id')->chunk(100) as $chunk) {
                ...
                Log::error('CetakBatch Error:', ['error' => $e->getMessage()]);
                return back()->with('error', 'Terjadi kesalahan, tapi server tidak dihentikan. Coba rentang lebih kecil.');
            }
        } else {
