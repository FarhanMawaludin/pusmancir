...
@section('content')
    <form method="POST" action="{{ route('admin.buku-elektronik.update', $buku->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="space-y-4">
            ...
            <div class="border-gray-900/10 pb-8 p-4 bg-white border border-gray-200 rounded">
                ...
                <div class="mt-4">
                    ...
                    <input type="text" name="kelas" id="kelas" value="{{ old('kelas', $buku->kelas) }}"
                        class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-text shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                        required>
                    @error('kelas')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="mt-4">
                    ...
                    <input type="text" name="kurikulum" id="kurikulum" value="{{ old('kurikulum', $buku->kurikulum) }}"
                        class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-text shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    @error('kurikulum')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mt-4">
                    ...
                    @if ($buku->pdf_path)
                        <p class="text-sm mt-1">Saat ini:
                            <a href="{{ asset($buku->pdf_path) }}" target="_blank" class="text-blue-600 underline">
                                Lihat File
                            </a>
                        </p>
                    @endif
                ...
                <div class="mt-4">
                    ...
                    @if ($buku->cover_image)
                        <p class="text-sm mt-1">Saat ini:
                            <a href="{{ asset($buku->cover_image) }}" target="_blank" class="text-blue-600 underline">
                                Lihat File
                            </a>
                        </p>
                    @endif
