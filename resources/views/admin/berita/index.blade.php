...
@section('content')
    <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold text-text">Data Berita</h1>
    </div>

    <!-- Filter dan Tambah -->
    <div class="flex justify-between items-center mb-4">
        <form class="flex w-full max-w-lg" method="GET" action="{{ route('admin.berita.index') }}">
            <div class="flex w-full">
                ...
                <div id="dropdown"
                    ...
                    <ul class="py-2 text-sm text-text" aria-labelledby="dropdown-button">
                        <li><button type="button" data-value="judul"
                                class="category-btn w-full text-left px-4 py-2 hover:bg-gray-100">Judul</button></li>
                        <li><button type="button" data-value="penulis"
                                class="category-btn w-full text-left px-4 py-2 hover:bg-gray-100">Penulis</button></li>
                        <li><button type="button" data-value="status"
                                class="category-btn w-full text-left px-4 py-2 hover:bg-gray-100">Status</button></li>
                    </ul>
                </div>
        ...
        <!-- Tombol Tambah -->
        <button onclick="location.href='{{ route('admin.berita.create') }}'"
            ...
            <tbody>
                @forelse ($berita as $key => $item)
                    <tr class="bg-white border-b border-gray-200">
                        <td class="px-4 py-2">{{ $berita->firstItem() + $key }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center text-gray-500 py-4">Tidak ada berita tersedia.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
