...
<head>
    <meta charset="UTF-8">
    <title>Laporan Peminjaman Non Paket</title>
    <style>
        ...
        .meta-info { display: flex; justify-content: space-between; font-size: 9pt; color: #555; margin-bottom: 5px; }
...
<body>
    ...
    {{-- Judul --}}
    <div class="judul-laporan">Laporan Peminjaman Non Paket</div>
        ...
        <tbody>
            @forelse($data as $key => $item)
                ...
                <tr>
                    ...
                    <td>{{ $item->eksemplar->inventori->judul_buku ?? '-' }}</td>
                    <td class="center">{{ $pinjam->tanggal_pinjam ? \Carbon\Carbon::parse($pinjam->tanggal_pinjam)->format('d-m-Y') : '-' }}</td>
                    <td class="center">{{ $pinjam->tanggal_kembali ? \Carbon\Carbon::parse($pinjam->tanggal_kembali)->format('d-m-Y') : '-' }}</td>
                    <td class="center">
            ...
            @empty
                <tr>
                    <td colspan="8" style="text-align: center; padding: 20px; color: #999;">Tidak ada data peminjaman ditemukan.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
