...
@section('content')
    <form method="POST" action="{{ route('admin.jenis-media.store') }}">
        @csrf
        <div class="space-y-4">
            ...
            <div class=" border-gray-900/10 pb-8 p-4  bg-white border border-gray-200 rounded">
                <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
                    <div class="sm:col-span-full">
                        <label for="nama_kelas" class="block text-sm/6 font-medium text-text">Nama Jenis Media</label>
                        <div class="mt-2">
                            <input type="text" name="nama_jenis_media" id="nama_jenis_media" value="{{ old('nama_jenis_media') }}"
                ...
                <div class="mt-6 flex items-center justify-start gap-x-4">
                    <button type="button"
                        ...
                        onclick="location.href='{{ route('admin.jenis-media.index') }}'">Batal</button>
                    <button type="submit"
