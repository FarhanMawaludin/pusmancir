...
@section('content')
    <div class="flex justify-between items-center mb-4">
        ...
        <button id="dropdownDividerButton" data-dropdown-toggle="dropdownDivider"
            onclick="location.href='{{ route('admin.sekolah.create') }}'" type="button"
        ...
        <table class="min-w-full text-sm text-left text-text">
            <thead class="text-xs uppercase bg-gray-100 text-text">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">Kode Sekolah</th>
                    <th scope="col" class="px-6 py-3">Aksi</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($sekolah as $key => $sekolahItem)
                    <tr class="bg-white border-b border-gray-200">
                        <td class="px-6 py-4">{{ $sekolah->firstItem() + $key }}</td>
                        <td class="px-6 py-4">
                            <div class="min-w-0">
                        ...
                        <td class="px-6 py-4">
                            <div
                                ...
                                <!-- Hapus -->
                                <form action="{{ route('admin.sekolah.destroy', $sekolahItem->id) }}" method="POST"
                                    class="inline" id="delete-form-{{ $sekolahItem->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
                                        class="inline-flex items-center bg-red-500 text-white px-3 py-2 rounded hover:bg-red-700 transition btn-delete"
                                        data-id="{{ $sekolahItem->id }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 md:mr-1" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
