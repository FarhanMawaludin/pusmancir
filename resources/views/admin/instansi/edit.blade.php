...
@section('content')
    <form method="POST" action="{{ route('admin.instansi.update', $instansi->id) }}" >
        ...
        @method('PUT')
        <div class="space-y-4">
            <h2 class="text-[28px] font-semibold text-text">Formulir Kode Instansi</h2>
            <div class=" border-gray-900/10 pb-8 p-4  bg-white border border-gray-200 rounded">
                <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
                    <div class="sm:col-span-3">
                        <label for="nama_kelas" class="block text-sm/6 font-medium text-text">Nama Lengkap Instansi</label>
                        <div class="mt-2">
                            <input type="text" name="nama_lengkap" id="nama_lengkap" value="{{ old('nama_lengkap', $instansi->nama_lengkap) }}"
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <div class="mt-2">
                            <input type="text" name="kode" id="kode" value="{{ old('kode', $instansi->kode) }}"
                                autocomplete="given-name"
                                class="mt-2 block w-full rounded-md bg-white px-3 py-1.5 text-base text-text 
                                border border-gray-300 placeholder:text-gray-400
                                focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:border-indigo-600 sm:text-sm"
                ...
                <div class="mt-6 flex items-center justify-start gap-x-4">
                    <button type="button"
                        ...
                        onclick="location.href='{{ route('admin.instansi.index') }}'">Batal</button>
                    <button type="submit"
