    ...
    <div class="flex justify-between items-center ">
        ...
        <section class="mb-2">
            ...
            <form method="GET" action="{{ route('admin.pengembalian.index') }}" class="mb-4 flex items-center gap-2">
                <input type="text" name="search" value="{{ request('search') }}" class="block w-full rounded-md bg-white px-2 py-1.5 text-base text-text outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
                ...
                @forelse ($pengembalian as $key => $pengembalianItem)
                    <tr class="bg-white border-b border-gray-200">
                        ...
                        <td class="px-6 py-4">
                            <div class="min-w-0">
                                <div class="font-medium md:text-base break-words truncate md:whitespace-normal">
                                    {{ $pengembalianItem->peminjaman->anggota->user->name }}
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">{{ $pengembalianItem->eksemplar->inventori->judul_buku }}</td>
                        <td class="px-6 py-4">
                            {{ \Carbon\Carbon::parse($pengembalianItem->peminjaman->tanggal_pinjam)->format('d-m-Y') }}
                        </td>
                        <td class="px-6 py-4">
                            {{ \Carbon\Carbon::parse($pengembalianItem->peminjaman->tanggal_kembali)->format('d-m-Y') }}
                        </td>
                        @php
                            $isTerlambat =
                                $pengembalianItem->eksemplar->status === 'dipinjam' &&
                                \Carbon\Carbon::now()->gt(
                                    \Carbon\Carbon::parse($pengembalianItem->peminjaman->tanggal_kembali),
                                );
                        @endphp
                        ...
                        <td class="px-6 py-4">
                            @if ($isTerlambat)
                                <span
                                    class="px-3 py-1 text-sm rounded-full border text-white border-red-600 bg-red-600 font-semibold">
                                    Terlambat
                                </span>
                            @else
                                <span
                                    ...
                                    {{ ucfirst($pengembalianItem->eksemplar->status) }}
                                </span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <form id="terima-form-{{ $pengembalianItem->id }}"
                                action="{{ route('admin.pengembalian.update', $pengembalianItem->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <button type="button"
