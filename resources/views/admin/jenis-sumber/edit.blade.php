...
@section('content')
    <form method="POST" action="{{ route('admin.jenis-sumber.update', $jenis_sumber->id) }}" class="space-y-8 divide-y divide-gray-900/10')  }}">
        @csrf
        @method('PUT')
        <div class="space-y-4">
            <h2 class="text-[28px] font-semibold text-text">Formulir Jenis Sumber</h2>
            <div class=" border-gray-900/10 pb-8 p-4  bg-white border border-gray-200 rounded">
                <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
                    <div class="sm:col-span-full">
                        <label for="jenis_sumber" class="block text-sm/6 font-medium text-text">Nama Sumber</label>
                        <div class="mt-2">
                            <input type="text" name="nama_sumber" id="nama_sumber" value="{{ old('nama_sumber' , $jenis_sumber->nama_sumber) }}"
                ...
                <div class="mt-6 flex items-center justify-start gap-x-4">
                    <button type="button"
                        ...
                        onclick="location.href='{{ route('admin.jenis-sumber.index') }}'">Batal</button>
                    <button type="submit"
