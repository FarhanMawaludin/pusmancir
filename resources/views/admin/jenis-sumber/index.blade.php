...
@section('content')
    <div class="flex justify-between items-center mb-4">
        <!-- Judul -->
        <h1 class="text-2xl font-bold text-text">Data Jenis Sumber</h1>

        <!-- Tombol Tambah -->
        <button id="dropdownDividerButton" data-dropdown-toggle="dropdownDivider"
            onclick="location.href='{{ route('admin.jenis-sumber.create') }}'" type="button"
    ...
    <div class="overflow-x-auto relative rounded border border-gray-200">
        <table class="min-w-full text-sm text-left text-text">
            <thead class="text-xs uppercase bg-gray-100 text-text">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">Nama Sumber</th>
                    <th scope="col" class="px-6 py-3">Aksi</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($jenis_sumber as $key => $jenisSumberItem)
                    <tr class="bg-white border-b border-gray-200">
                        <td class="px-6 py-4">{{ $jenis_sumber->firstItem() + $key }}</td>
                        <td class="px-6 py-4">
                            <div class="min-w-0">
                                <div class="font-medium md:text-base break-words truncate md:whitespace-normal">
                                    {{ $jenisSumberItem->nama_sumber }}
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div
                                ...
                                <!-- Edit -->
                                <a href="{{ route('admin.jenis-sumber.edit', $jenisSumberItem->id) }}"
                                ...
                                <!-- Hapus -->
                                <form action="{{ route('admin.jenis-sumber.destroy', $jenisSumberItem->id) }}" method="POST"
                                    class="inline" id="delete-form-{{ $jenisSumberItem->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
                                        class="inline-flex items-center bg-red-500 text-white px-3 py-2 rounded hover:bg-red-700 transition btn-delete"
                                        data-id="{{ $jenisSumberItem->id }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 md:mr-1" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <!-- Pagination -->
        <div class="p-4">
            {{ $jenis_sumber->links('pagination::tailwind') }}
        </div>
    </div>
