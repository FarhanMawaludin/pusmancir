...
@section('content')
    <div class="flex justify-between items-center mb-4">
        <!-- Judul -->
        <h1 class="text-2xl font-bold text-text">Data Kode Jenis Surat</h1>

        <!-- Tombol Tambah -->
        <button id="dropdownDividerButton" data-dropdown-toggle="dropdownDivider"
            onclick="location.href='{{ route('admin.kode-jenis-surat.create') }}'" type="button"
    ...
    <div class="overflow-x-auto relative rounded border border-gray-200">
        <table class="min-w-full text-sm text-left text-text">
            <thead class="text-xs uppercase bg-gray-100 text-text">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">Nama Kode</th>
                    <th scope="col" class="px-6 py-3">Kode</th>
                    <th scope="col" class="px-6 py-3">Aksi</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($jenis_surat as $key => $jenisSuratItem)
                    <tr class="bg-white border-b border-gray-200">
                        <td class="px-6 py-4">{{ $jenis_surat->firstItem() + $key }}</td>
                        <td class="px-6 py-4">
                            <div class="min-w-0">
                                <div class="font-medium md:text-base break-words truncate md:whitespace-normal">
                                    {{ $jenisSuratItem->nama_surat }}
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div
                                ...
                                <!-- Edit -->
                                <a href="{{ route('admin.kode-jenis-surat.edit', $jenisSuratItem->id) }}"
                                ...
                                <!-- Hapus -->
                                <form action="{{ route('admin.kode-jenis-surat.destroy', $jenisSuratItem->id) }}" method="POST"
                                    class="inline" id="delete-form-{{ $jenisSuratItem->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
                                        class="inline-flex items-center bg-red-500 text-white px-3 py-2 rounded hover:bg-red-700 transition btn-delete"
                                        data-id="{{ $jenisSuratItem->id }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 md:mr-1" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-4 text-center text-gray-500">
                            Tidak ada kode jenis surat ditemukan.
                        </td>
                    </tr>
                @endforelse
            </tbody>
        ...
        <!-- Pagination -->
        <div class="p-4">
            {{ $jenis_surat->links('pagination::tailwind') }}
        </div>
    </div>
