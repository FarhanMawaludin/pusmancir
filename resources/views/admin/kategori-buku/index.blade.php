...
@section('content')
    <div class="flex justify-between items-center mb-4">
        <!-- Judul -->
        <h1 class="text-2xl font-bold text-text">Data Kategori Buku</h1>

        <!-- Tombol Tambah -->
        <button id="dropdownDividerButton" data-dropdown-toggle="dropdownDivider"
            onclick="location.href='{{ route('admin.kategori-buku.create') }}'" type="button"
    ...
    <div class="overflow-x-auto relative rounded border border-gray-200">
        <table class="min-w-full text-sm text-left text-text">
            ...
            <tbody>
                @forelse ($kategori_buku as $key => $kategoriBukuItem)
                    <tr class="bg-white border-b border-gray-200">
                        <td class="px-6 py-4">{{ $kategori_buku->firstItem() + $key }}</td>
                        <td class="px-6 py-4">
                            <div class="min-w-0">
                                <div class="font-medium md:text-base break-words truncate md:whitespace-normal">
                                    {{ $kategoriBukuItem->nama_kategori }}
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div
                                ...
                                <!-- Edit -->
                                <a href="{{ route('admin.kategori-buku.edit', $kategoriBukuItem->id) }}"
                                ...
                                <!-- Hapus -->
                                <form action="{{ route('admin.kategori-buku.destroy', $kategoriBukuItem->id) }}" method="POST"
                                    class="inline" id="delete-form-{{ $kategoriBukuItem->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
                                        class="inline-flex items-center bg-red-500 text-white px-3 py-2 rounded hover:bg-red-700 transition btn-delete"
                                        data-id="{{ $kategoriBukuItem->id }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 md:mr-1" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
