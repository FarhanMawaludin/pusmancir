...
@section('content')
    ...
    <form id="form-bulk-terima" action="{{ route('admin.pengembalian-paket.bulkUpdate') }}" method="POST" class="hidden">
        @csrf
        @method('PUT')
            ...
            <tbody>
                @forelse ($pengembalian as $key => $pengembalianItem)
                    ...
                    <tr class="bg-white border-b border-gray-200">
                        <td class="px-6 py-4">
                            @if ($status === 'berhasil')
                                <input type="checkbox" name="paket_ids[]" value="{{ $pengembalianItem->id }}"
    ...
    <script>
        ...
        if (btnBulkTerima) {
            btnBulkTerima.addEventListener('click', () => {
                const selected = Array.from(document.querySelectorAll('.paket-checkbox:checked'));
                if (selected.length === 0) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Oops!',
                        text: 'Pilih minimal satu pengembalian paket terlebih dahulu.'
                    });
                    return;
                }

                Swal.fire({
                    title: 'Terima ' + selected.length + ' Pengembalian Paket Terpilih?',
                    text: 'Semua pengembalian paket yang dipilih akan diproses menjadi selesai dan stok dikembalikan.',
                    icon: 'question',
                    showCancelButton: true,
