...
@section('content')
    <form method="POST" action="{{ route('admin.sumber.update', $sumber->id) }}" class="space-y-8 divide-y divide-gray-900/10') }}">
        @csrf
        @method('PUT')
        <div class="space-y-4">
            <h2 class="text-[28px] font-semibold text-text">Formulir Sumber</h2>
            <div class=" border-gray-900/10 pb-8 p-4  bg-white border border-gray-200 rounded">
                <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
                    <div class="sm:col-span-full">
                        <label for="nama_kelas" class="block text-sm/6 font-medium text-text">Nama Sumber</label>
                        <div class="mt-2">
                            <input type="text" name="nama" id="nama" value="{{ old('nama', $sumber->nama) }}"
                ...
                <div class="mt-6 flex items-center justify-start gap-x-4">
                    <button type="button"
                        ...
                        onclick="location.href='{{ route('admin.sumber.index') }}'">Batal</button>
                    <button type="submit"
