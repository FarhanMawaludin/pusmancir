...
@section('content')
    <form method="POST" action="{{ route('admin.eksemplar.update', $eksemplar->id) }}">
        @csrf
        @method('PUT')
        <div class="space-y-4">
            <h2 class="text-[28px] font-semibold text-text">Formulir Jenis Media</h2>
            <div class=" border-gray-900/10 pb-8 p-4  bg-white border border-gray-200 rounded">
                <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
                    <div class="sm:col-span-full">
                        <label for="status" class="block text-sm/6 font-medium text-text">Status Buku</label>
                        <div class="mt-2">
                            ...
                            focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:border-indigo-600 sm:text-sm">
                                <option value="tersedia"
                                    {{ old('status', $eksemplar->status) == 'tersedia' ? 'selected' : '' }}>Tersedia
                                </option>
                                <option value="dipinjam"
                                    {{ old('status', $eksemplar->status) == 'dipinjam' ? 'selected' : '' }}>Dipinjam
                                </option>
                                <option value="rusak" {{ old('status', $eksemplar->status) == 'rusak' ? 'selected' : '' }}>
                                    Rusak</option>
                                <option value="hilang"
                                    {{ old('status', $eksemplar->status) == 'hilang' ? 'selected' : '' }}>Hilang</option>
                            </select>
                            @error('status')
                ...
                <div class="mt-6 flex items-center justify-start gap-x-4">
                    <button type="button"
                        ...
                        onclick="location.href='{{ route('admin.inventori.show', $eksemplar->id_inventori) }}'">Batal</button>
                    <button type="submit"
